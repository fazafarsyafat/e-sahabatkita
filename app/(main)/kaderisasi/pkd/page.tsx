...
export default function PkdMateriPage() {
  ...
  useEffect(() => {
    const fetchMateri = async () => {
      try {
        const res = await fetch('/api/materi?jenjang=PKD');
    ...
    const fetchPrograms = async () => {
      try {
        ...
        if(Array.isArray(data)) {
          setPrograms(data.filter(p => p.jenis === 'PKD'));
        }
      } catch (e) {
        console.error(e);
      }
    ...
    <div className="max-w-4xl mx-auto space-y-12">
      ...
      <section>
        <div className="text-center mb-10">
          <div className="w-16 h-16 bg-yellow-100 dark:bg-yellow-900/30 text-yellow-600 rounded-2xl mx-auto flex items-center justify-center mb-4">
            <GraduationCap size={32} />
          </div>
          <h2 className="text-2xl font-black text-gray-900 dark:text-white mb-2">Materi Induk PKD</h2>
          <p className="text-gray-500 dark:text-gray-400">Unduh dan pelajari modul-modul wajib Pelatihan Kader Dasar (PKD).</p>
        </div>
            ...
            materi.map((item, i) => (
              ...
              >
                <div className="flex items-center gap-4">
                  ...
                  <div>
                    <h4 className="font-bold text-gray-900 dark:text-white text-sm group-hover:text-yellow-600 transition-colors">{item.judul}</h4>
                    <div className="text-xs text-gray-500 font-medium">{item.format} • {item.ukuran}</div>
                  </div>
                </div>
                <a href={item.fileUrl} target="_blank" className="btn-ghost p-2 rounded-full text-gray-400 hover:text-yellow-600 hover:bg-yellow-50">
                  <Download size={18} />
                </a>
              </motion.div>
            ))
      ...
      <section className="pt-10 border-t border-gray-100 dark:border-gray-800">
        <div className="text-center mb-8">
          <h3 className="text-xl font-bold text-gray-900 dark:text-white mb-2">Jadwal Pelaksanaan PKD</h3>
          <p className="text-sm text-gray-500 dark:text-gray-400">Daftar kegiatan PKD yang sedang atau akan berlangsung.</p>
        </div>
        ...
        <div className="grid sm:grid-cols-2 gap-6">
          {loadingPrograms ? (
            ...
            <div className="col-span-2 text-center py-10 bg-white dark:bg-gray-900 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-800 text-gray-500">Belum ada jadwal PKD saat ini.</div>
          ) : (
            programs.map((program, i) => (
              ...
              >
                ...
                <p className="text-sm text-gray-600 dark:text-gray-400 mb-6 line-clamp-2">
                  {program.deskripsi || 'Kegiatan kaderisasi tingkat menengah PMII.'}
                </p>
